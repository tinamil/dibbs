                                         ...
                                         uint8_t* __restrict__ results)
{
  ...
  constexpr size_t SPLIT = NUM_INTS_PER_PANCAKE - 1;
  __shared__ uint32_t sB[TILE_WIDTH_Y][SPLIT];
  ...
  for(uint32_t by = blockIdx.y; by < max_b; by += gridDim.y) {
    ...
    for(int tidx = threadIdx.x; tidx < SPLIT; tidx += blockDim.x) {
      if(row < rows_b) {
    ...
    for(uint32_t bx = blockIdx.x; bx < max_a; bx += gridDim.x) {
      ...
      if(row < rows_b && col < rows_a) {
        ...
        uint32_t tmpB = 0;
        uint32_t tmpMin;
        static_assert(NUM_GAP_INTS <= SPLIT, "Failure in loop order.");
        #pragma unroll
        ...
        #pragma unroll
        for(int i = NUM_GAP_INTS; i < SPLIT; ++i) {
          uint32_t A = sA[i][threadIdx.x];
          uint32_t B = sB[threadIdx.y][i];
          tmpMin += __popc(A & B);
        }
        #pragma unroll
        for(int i = SPLIT; i < NUM_INTS_PER_PANCAKE; ++i) {
          uint32_t A = sA[i][threadIdx.x];
          uint32_t B = hash_b[row * NUM_INTS_PER_PANCAKE + i];// sB[threadIdx.y][i];
          tmpMin += __popc(A & B);
        }
                              ...
                              uint8_t* __restrict__ mult_results)
{
  ...
  constexpr uint32_t MAX_BLOCKS_X = 8196;
  constexpr uint32_t MAX_BLOCKS_Y = 1024;
