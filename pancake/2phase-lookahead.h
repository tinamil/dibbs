...
namespace TWO_PHASE
{
  ...
  inline uint32_t update_hash_value(const uint8_t seq[], int i, uint32_t hash_value)
  {
    ...
    if(hash_value > hash_table::hash(pi, pi1))
      ...
      hash_value = hash_value - hash_table::hash(pi, pi1) + UINT32_MAX;
    hash_value = (hash_value + hash_table::hash(p1, pi1)) % UINT32_MAX;
  ...
  class TwoPhase
  {
    ...
    typedef tsl::hopscotch_map<uint32_t, const Pancake*> neighbor_map;
    ...
    bool expand_node(const Pancake* next_val, hash_set& hash, hash_set& closed, const hash_set& other_hash, pancake_triple& data, neighbor_map& neighbors, neighbor_map& other_neighbors)
    {
      ...
      uint32_t starting_hash = hash_table::hash(next_val->source);
      for(int i = 2; i <= NUM_PANCAKES && UB > lbmin; ++i)
      {
        Pancake new_action = next_val->apply_action(i);
        uint32_t new_hash = update_hash_value(next_val->source, i, starting_hash);
        ...
        if constexpr(LOOKAHEAD)
        {
          if(!already_matched && lbmin + 1 < UB)
          {
            if(other_neighbors.count(new_hash) > 0)
            {
              ...
              if(other_neighbor->g + new_action.g + 1 < UB && compare_one_off(other_neighbor, &new_action))
              {
                expand_node(&new_action, hash, closed, other_hash, data, neighbors, other_neighbors);
              }
            }
          }
          for(int j = 2; j <= NUM_PANCAKES; ++j)
          {
            uint32_t new_neighbor_hash = update_hash_value(ptr->source, j, new_hash);
