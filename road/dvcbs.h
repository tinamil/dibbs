...
class Dvcbs
{
  ...
  typedef std::set<const Node*, NodeFSortLowG> waiting_set;
  typedef std::set<const Node*, NodeGSortLow> ready_set;
  ...
  bool expand_node(const Node* next_val, ready_set& ready, waiting_set& waiting, hash_set& hash, hash_set& closed, const hash_set& other_hash)
  {
  ...
  bool expand_node_forward(const Node* next_val)
  {
    return expand_node(next_val, open_f_ready, open_f_waiting, open_f_hash, closed_f, open_b_hash);
  }

  bool expand_node_backward(const Node* next_val)
  {
  ...
  std::tuple<double, size_t, size_t> run_search(Node start, Node goal)
  {
    ...
    lbmin = std::max(1ui32, std::max(start.h, goal.h));
    ...
    while(true)
    {
      ...
      std::vector<const Node*> nForward, nBackward;
      ...
      {
        std::unordered_map<const Node*, bool, NodeHash, NodeEqual> mapData;
  ...
  bool getVertexCover(std::vector<const Node*>& nextForward, std::vector<const Node*>& nextBackward)
  {
    while(true)
    {
