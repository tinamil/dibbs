...
#define  HASH_SIZE 400000000  // Must be a prime number.  Currently using linear
                              ...
                              // it must be a prime of the form 4j+3.  4000039=4*1000009+3  c=4*10000000+3   400000043=4*100000010+3   1000000007 = 4*250000001+3
...
public:
   Hash_table()   {n = 0; table = NULL; hash_value = NULL;}
   ~Hash_table()  {delete [] table; for(int i = 0; i < N_LOCATIONS; i++) delete [] hash_value[i]; delete [] hash_value;}
   void  initialize()   {  table = new hash_record[HASH_SIZE];
                           if(table == NULL) {
   ...
   void  clear()        {for(int i = 0; i < HASH_SIZE; i++) {table[i].clear();} n = 0;}
   ...
   void  initialize_hash_value()    {  double   seed;
                                       seed = 3.1567;
