      ...
      for (int face = 0; face < 6; ++face)
      {
        ...
        for (int rotation = 0; rotation < 3; ++rotation)
        {
          ...
          if (new_node->combined < next_node->combined) {
            std::cout << "Consistency error: " << unsigned(new_node->heuristic) << " < " << unsigned(next_node->heuristic)
              << " + depth change of " << unsigned(new_node->depth) << " to " << unsigned(next_node->depth) << std::endl;
          }
